...
function setCurrencyFromQuote() {
    if (Xrm.Page.getAttribute("new_invoiceid").getValue() && Xrm.Page.getAttribute("new_invoiceid").getValue()[0].id != "") {
        var id = Xrm.Page.getAttribute("new_invoiceid").getValue()[0].id;
        id = id.replace("{", "");
        id = id.replace("}", "");
        retrieveRecord(id, "QuoteSet", function (data) {
    ...
    function SetBankData() {
        if (GetFieldValue("new_bankid") == null) {
            var lookupData = new Array();
            var lookupItem = new Object();
            lookupItem.id = "388B3270-4B3F-E511-80E1-005056960005";
            lookupItem.typename = "new_bank";
            lookupItem.name = '“Muganbank” ASC-nin “Mətbuat” filialı';
            lookupData[0] = lookupItem;
            SetFieldValue("new_bankid", lookupData);
        }
    }
    ...
    function SetInvoiceWithNDS() {
        var tatotal_amount = GetFieldValue("totallineitemamount");

        var account = GetFieldValue("customerid");

        if (!account) {
            SetFieldValue("new_nds", 0);
            return;
        }
        ...
        retrieveRecord(id, "AccountSet", function (data) {
            if (data) {
                var nds_percent = data.new_plat_nds ? 18 : 0;

                var nds_amount = tatotal_amount * nds_percent / 100;

                SetFieldValue("new_nds", nds_amount);
            }
        }, function (e) { debugger; }, true, "?$select=new_plat_nds");
    }
