...
namespace SoftLine.UA.Notification.Service
{
    class AgreementNotification
    {
        ...
        public void CheckAgreementEve()
        {
            using (var orgContext = new OrganizationServiceContext(service))
            {
                var correntAgreement = (from c in orgContext.CreateQuery<new_supplementaryagreement>()
                                        ...
                                        select new ProxyToSendEmailExtended
                                        {
                                            ...
                                            agreementFin = (c.new_agreed == false && Equals(c.new_participationof, new OptionSetValue(100000000))) ? c.new_financedepartment : null,
                ...
                SendEmail(correntAgreement, service, 0);
            }
        }

        public void CheckAgreementToday()
        {
            using (var orgContext = new OrganizationServiceContext(service))
            {
                var correntAgreement = (from c in orgContext.CreateQuery<new_supplementaryagreement>()
                ...
                SendEmail(correntAgreement, service, 1);
            }
        }

        private void SendEmail(List<ProxyToSendEmailExtended> listAgreement, IOrganizationService service, byte whatDay)
        {
            ...
            foreach (var item in Agreements)
            {
                ...
                var current = from p in listAgreement
                ...
                {
                    //https://ukraine.crm.softlinegroup.com/userdefined/edit.aspx?etc=1088&id=%{0}%7d
                    link += string.Format("<p class='MsoNormal'><i><u><span lang='RU' style='color:rgb(0,112,192)'>" +
                        /*"<a href=https://ukraine.crm.softlinegroup.com/main.aspx?etc=SalesOrder&extraqs=%3f_gridType%3d4212%26etc%3d4212%26id%3d%257b{0}" +
                            "%257d%26preloadcache%3d1406639059288%26rskey%3d307497509&histKey=417657088&newWindow=true&pagetype=entityrecord>{1}</a></span></u></i></p>",*/
                                            "<a href=https://ukraine.crm.softlinegroup.com/userdefined/edit.aspx?etc=1088&id=%{0}%7d>{1}</a></span></u></i></p>",
...
<p class='MsoNormal'><span lang='RU'>Уважаемый коллега, срок согласования Дополнительного соглашения(ий) истекает завтра. Просьба согласовать Договор либо внести комментарии.</span><o:p></o:p></p>          
<p class='MsoNormal'><i><u><span lang='RU' style='color: rgb(0, 112, 192);'>{0}</span></u></i></p>
                    ...
                    case 1:
                        mail.Description = string.Format(@"<div id=':3ua' class='Am Al editable LW-avf' hidefocus='true' aria-label='Текст' 
g_editable='true' role='textbox' contenteditable='true' tabindex='1' itacorner='6,7:1,1,0,0' style='direction: ltr; min-height: 236px;'>
<p class='MsoNormal'><span lang='RU'>Уважаемый коллега, срок согласования Дополнительного соглашения(ий) истекает сегодня. Просьба согласовать Договор либо внести комментарии.</span><o:p></o:p></p>          
<p class='MsoNormal'><i><u><span lang='RU' style='color: rgb(0, 112, 192);'>{0}</span></u></i></p>
                                                            <br></div>", link.ToString());
                        break;
                }
